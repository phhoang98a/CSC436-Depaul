...
const Product = class Product {
  constructor(input){
    ...
    this.name = infos[0];
    this.price = Number(infos[1]);
    this.availability = infos[2];
  }
  /**
   * *****************
   * ***** TODO ******
  ...
  static printProducts = (products) => { 
    products.forEach((product) => {
      let result = "Product: "+product.name+", Cost: "+new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(product.price)+", Availability: ";
      product.availability==="In Stock" ? result = result + "Yes" : result = result + "No";
      console.log(result);
    });
  }
